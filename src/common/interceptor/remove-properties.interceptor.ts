...
export class RemovePropertiesInterceptor implements NestInterceptor {
  ...
  intercept(
    ...
    // Modify or filter the response before it is sent to the client
    return next.handle().pipe(
      map((data) => {
        // Remove specified properties from the response object
        if (data) {
      ...
      if (obj.hasOwnProperty(prop)) {
        if (propertiesToRemove.includes(prop)) {
          ...
          if (Array.isArray(obj[prop])) {
            ...
            // If it's an object, continue recursively
